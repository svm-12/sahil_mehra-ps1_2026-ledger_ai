...
def extract_document(req: schemas.DocumentExtractRequest, db: Session = Depends(get_db)):
    try:
        ...
        doc = models.Document(
            ...
            is_subscription=1 if extracted.is_subscription else 0,
            total_amount=total,
...
def update_document(doc_id: int, req: schemas.DocumentUpdate, db: Session = Depends(get_db)):
    ...
    doc.is_subscription = (1 if req.is_subscription else 0) if req.is_subscription is not None else doc.is_subscription
...
def create_document_manual(req: schemas.DocumentCreate, db: Session = Depends(get_db)):
    doc = models.Document(
        ...
        is_subscription=1 if req.is_subscription else 0,
